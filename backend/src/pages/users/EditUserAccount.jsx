...
const EditUserAccount = () => {
   ...
   * users context
   */
  const { status, error, singleUser } = useSelector(usersSelect);

  // effect handler for singleUser getting
  useEffect(() => {
    dispatch(getSingleUserAccount(id));
  }, [dispatch, id]);
  ...
  const { input, handleInputChange, setInput } = useForm({
    name: singleUser.meta?.name
  });

  /**
   * set user form data by editable user data
   */
//   useEffect(() => {
//     setInput((prev) => ({
//       ...prev,
...
//       name: singleUser.meta?.name,
//       phone: singleUser.meta?.phone,
//       role: singleUser.meta?.role,
//     }));
//   });
